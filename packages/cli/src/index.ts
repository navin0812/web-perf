...
import {
  checkThresholds,
  formatThresholdViolations,
} from "./threshold-checker.js";
...
export async function runAudit(options: CliOptions): Promise<number> {
  ...
  try {
    ...
    console.error(
      chalk.red("Failed to run audits:"),
  ...
  try {
    ...
    console.error(
      chalk.red("Failed to generate reports:"),
      error instanceof Error ? error.message : error,
    );
    return 1;
  }
...
async function generateReports(
  ...
  const formats =
    options.format === "all" ? ["terminal", "json", "html"] : [options.format];
  ...
  for (const format of formats) {
    if (format === "terminal") {
      ...
      if (content) {
        ...
        console.log(
          chalk.green(`✓ ${format.toUpperCase()} report saved: ${filePath}`),
        );
      }
    }
  }
...
function printSummary(report: AuditReport): void {
  ...
  console.log(
    `  Critical: ${chalk.red(summary.bySeverity.critical.toString())}`,
  );
  console.log(
    `  Serious: ${chalk.yellow(summary.bySeverity.serious.toString())}`,
  );
  console.log(
    `  Moderate: ${chalk.blue(summary.bySeverity.moderate.toString())}`,
  );
  ...
  console.log(
    `  Duration: ${chalk.gray(`${(report.duration / 1000).toFixed(2)}s`)}`,
  );
}

...
program
  ...
  .option(
    "-f, --format <format>",
    "Output format: json, html, terminal, all",
    "terminal",
  )
  .option(
    "-o, --output-dir <dir>",
  ...
  .option(
    "-t, --threshold <json>",
    'Threshold configuration as JSON (e.g., \'{"critical":0,"serious":5}\')',
  )
  .option(
    "-s, --skip-audits <audits>",
...
// Parse threshold
let threshold: ThresholdConfig | undefined;
if (options.threshold) {
  try {
    ...
    for (const key of Object.keys(threshold || {})) {
      ...
      if (
        threshold &&
        typeof threshold[key as keyof ThresholdConfig] !== "number"
      ) {
    ...
    console.error(
      chalk.red(
        `Error: Invalid threshold JSON. ${error instanceof Error ? error.message : "Unknown error"}`,
      ),
    );
    process.exit(1);
  }
...
// Parse skip audits
let skipAudits: string[] | undefined;
if (options.skipAudits) {
  skipAudits = options.skipAudits.split(",").map((s: string) => s.trim());
  const validAudits = [
    "accessibility",
    "performance",
    "seo",
  ...
  if (skipAudits) {
    for (const audit of skipAudits) {
      if (!validAudits.includes(audit)) {
        console.error(
          chalk.red(
            `Error: Invalid audit name "${audit}". Must be one of: ${validAudits.join(", ")}`,
          ),
        );
        process.exit(1);
      }

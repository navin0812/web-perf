...
const FIX_GUIDANCE = {
    ...
    "button-name": (node) => node.html?.includes("aria-label")
    ...
    "color-contrast": () => "Increase contrast ratio to at least 4.5:1 for normal text or 3:1 for large text",
    ...
    "heading-order": () => "Ensure headings follow a logical order without skipping levels",
    region: () => "Wrap content in landmark regions (main, nav, header, footer)",
    "aria-valid-attr": () => "Fix or remove invalid ARIA attributes",
    "aria-required-attr": () => "Add required ARIA attributes for the element role",
    "aria-roles": () => "Use a valid ARIA role value",
    "meta-viewport": () => "Allow users to zoom by setting viewport meta tag correctly",
    ...
    "scrollable-region-focusable": () => "Make scrollable regions keyboard accessible with tabindex",
    "frame-focusable-content": () => "Ensure iframe content is keyboard accessible",
    ...
    "no-autoplay-audio": () => "Remove autoplay or provide controls to pause audio",
    "object-alt": () => "Provide alternative text for object elements",
    "svg-img-alt": () => 'Add accessible name to SVG with role="img"',
    "td-headers-attr": () => "Use headers attribute to associate data cells with headers",
    "th-has-data-cells": () => "Ensure table headers have associated data cells",
    "scope-attr-valid": () => "Use valid scope values: row, col, rowgroup, colgroup",
    "table-fake-caption": () => "Use <caption> element instead of fake caption cells",
    "definition-list": () => "Ensure definition lists only contain dt and dd elements",
    ...
    blink: () => "Remove <blink> element - it is deprecated and causes accessibility issues",
};
/**
        ...
        for (const violation of results.violations) {
            for (const node of violation.nodes) {
                ...
                const issue = {
                    ...
                    fix: {
                        description: FIX_GUIDANCE[violation.id]?.(node) || "See documentation",
...
function mapAxeSeverityToSeverity(impact) {
    switch (impact) {
        case "critical":
            return "critical";
        case "serious":
...
function generateCodeFix(ruleId, node) {

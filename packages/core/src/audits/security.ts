 ...
 * Run security audit
 */
export async function auditSecurity(page: BrowserPage): Promise<{
  issues: Issue[];
  passed: PendingCheck[];
}> {
  const issues: Issue[] = [];
  ...
  if (httpsCheck) issues.push(httpsCheck);
  else
    passed.push({
      id: "sec-https",
      name: "HTTPS Enabled",
  ...
  if (pwCheck) issues.push(pwCheck);
  else
    passed.push({
      id: "sec-password",
  ...
  if (extLinkCheck) issues.push(extLinkCheck);
  else
    passed.push({
      id: "sec-ext-links",
...
function checkMixedContent(page: BrowserPage): Issue[] {
  const issues: Issue[] = [];
  const mixedElements = [
...
async function checkSecurityHeaders(page: BrowserPage): Promise<Issue[]> {
  const issues: Issue[] = [];
  const headers = [
...
function getHeaderFixCode(headerName: string): string {
  switch (headerName) {
...
function checkPasswordFieldsSecurity(page: BrowserPage): Issue | null {
  const passwordFields = page.document.querySelectorAll(
    'input[type="password"]',
  );
...
function checkExternalLinks(page: BrowserPage): Issue | null {
  const externalLinks = Array.from(
    page.document.querySelectorAll('a[target="_blank"]'),
  ).filter((link): link is HTMLAnchorElement => {
    if (!(link instanceof page.window.HTMLAnchorElement)) return false;
  ...
  if (externalLinks.length > 0) {
    return {
      ...
      description:
        'Links with target="_blank" should prevent reverse tabnabbing',

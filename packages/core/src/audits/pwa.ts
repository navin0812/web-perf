 ...
 */
async function checkManifest(page: BrowserPage): Promise<Issue[]> {
  const issues: Issue[] = [];
  ...
  if (!manifestLink) {
    issues.push({
      ...
      description:
        "A web app manifest is required for Progressive Web Apps. It defines how your app appears to users and enables installation.",
  ...
  issues.push({
    ...
    description:
      "Ensure your manifest.json includes required properties: name, icons (192x192 and 512x512), start_url, display, and theme_color.",
    helpUrl: "https://web.dev/add-manifest/",
    wcag: {
      id: "N/A",
      level: "AAA",
 ...
 */
function checkServiceWorker(page: BrowserPage): Issue | null {
  const document = page.document;
  ...
  if (!hasServiceWorker) {
    return {
      ...
      description:
        "Service workers enable offline functionality and are a core requirement for Progressive Web Apps. They allow your app to work without a network connection.",
 ...
 * Check HTTPS
 */
function checkHTTPS(page: BrowserPage): Issue | null {
  const url = page.url;
  ...
  if (!url.startsWith("https://")) {
    return {
      ...
      description:
        "Progressive Web Apps require HTTPS to ensure security. Service workers and many modern web APIs only work over secure connections.",
 ...
 */
function checkViewport(page: BrowserPage): Issue | null {
  const document = page.document;
  ...
  if (!viewport) {
    return {
      ...
      description:
        "The viewport meta tag is essential for responsive design and PWAs. It ensures your app displays correctly on mobile devices.",
  ...
  if (!content.includes("width=device-width")) {
    return {
      ...
      description:
        "The viewport should include width=device-width to ensure proper responsive behavior on mobile devices.",
      helpUrl: "https://web.dev/viewport/",
      wcag: {
        id: "N/A",
        level: "AAA",
 ...
 */
function checkAppleTouchIcon(page: BrowserPage): Issue | null {
  const document = page.document;
  ...
  if (!appleTouchIcon) {
    return {
      ...
      description:
        "Apple touch icons ensure your PWA looks good when added to iOS home screens. Recommended size is 180x180px.",
      helpUrl:
 ...
 * Check theme color
 */
function checkThemeColor(page: BrowserPage): Issue | null {
  const document = page.document;
  ...
  if (!themeColor) {
    return {
      ...
      description:
        "The theme-color meta tag customizes the browser UI color on mobile devices, providing a more native app-like experience.",
 ...
 */
export async function auditPWA(page: BrowserPage): Promise<{
  issues: Issue[];
  passed: PendingCheck[];
}> {
  const issues: Issue[] = [];

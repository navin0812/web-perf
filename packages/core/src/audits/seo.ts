...
function checkTitle(page: BrowserPage): Issue | null {
  ...
  if (length < 30 || length > 60) {
    return {
      ...
      description:
        "Page titles should be between 50-60 characters for optimal display",
      helpUrl:
        "https://developers.google.com/search/docs/appearance/title-link",
      wcag: {
        id: "2.4.2",
      ...
      fix: {
        description:
          "Adjust your title to be between 50-60 characters for better SEO",
...
function checkMetaDescription(page: BrowserPage): Issue | null {
  ...
  if (length < 120 || length > 160) {
    return {
      ...
      fix: {
        description:
          "Adjust your meta description to be between 150-160 characters",
...
function checkH1(page: BrowserPage): Issue[] {
  ...
  if (h1s.length === 0) {
    ...
    issues.push({
      ...
      fix: {
        description:
          "Keep only one H1 heading and convert others to H2 or lower",
...
function checkCanonical(page: BrowserPage): Issue | null {
...
function checkOpenGraph(page: BrowserPage): Issue[] {
  const issues: Issue[] = [];
...
function checkStructuredData(page: BrowserPage): Issue | null {
  const schema = page.document.querySelector(
 ...
 * Run SEO audit
 */
export async function auditSEO(page: BrowserPage): Promise<{
  issues: Issue[];
  passed: PendingCheck[];
}> {
  const issues: Issue[] = [];

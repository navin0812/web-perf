...
export function renderTerminalReport(
  ...
  // ANSI color codes
  const color = (code: string, str: string) => `\x1b[${code}m${str}\x1b[0m`;
  ...
  console.log(
    `  ${bold("Timestamp:")} ${new Date(report.timestamp).toLocaleString()}`,
  );
  ...
  console.log(
    `  ${bold("Passed Checks:")} ${color("32", report.passed.length.toString())}`,
  );
  ...
  Object.entries(report.summary.bySeverity).forEach(([sev, count]) => {
    ...
    console.log(
      `    ${color(sevColor, icon)} ${color(sevColor, sev.padEnd(10))}: ${color(sevColor, count.toString().padStart(3))} ${color(sevColor, bar)}`,
    );
  });
  ...
  Object.entries(report.summary.byCategory)
    ...
    .forEach(([cat, count]) => {
      ...
      console.log(
        `    ${cat.padEnd(12)}: ${count.toString().padStart(3)} ${color("34", bar)}`,
      );
    });
  ...
  if (threshold) {
    ...
    if (
      ...
      violations.push(
        `Critical: ${report.summary.bySeverity.critical} > ${threshold.critical}`,
      );
    }
    if (
      threshold.serious !== undefined &&
      report.summary.bySeverity.serious > threshold.serious
    ) {
      violations.push(
        `Serious: ${report.summary.bySeverity.serious} > ${threshold.serious}`,
      );
    }
    if (
      threshold.moderate !== undefined &&
      report.summary.bySeverity.moderate > threshold.moderate
    ) {
      violations.push(
        `Moderate: ${report.summary.bySeverity.moderate} > ${threshold.moderate}`,
      );
    }
    if (
      threshold.minor !== undefined &&
      report.summary.bySeverity.minor > threshold.minor
    ) {
      violations.push(
        `Minor: ${report.summary.bySeverity.minor} > ${threshold.minor}`,
      );
    }
  ...
  if (report.issues.length > 0) {
    ...
    report.issues.forEach((issue, index) => {
      ...
      if (issue.element) {
        ...
        console.log(
          `     ${dim("Selector:")} ${color("36", issue.element.selector)}`,
        );
        ...
        if (issue.element.html) {
          ...
          console.log(
            `     ${dim("HTML:")} ${color("90", htmlSnippet + truncated)}`,
          );
        }

        if (issue.element.failureSummary) {
          console.log(
            `     ${dim("Failure:")} ${color("33", issue.element.failureSummary)}`,
          );
        }
      }
      ...
      if (
        ...
        console.log(
          `     ${dim("WCAG:")} ${issue.wcag.id} (Level ${issue.wcag.level}) - ${issue.wcag.name}`,
        );
      }
  ...
  if (report.passed.length > 0) {
    ...
    const grouped = report.passed.reduce(
      (acc, check) => {
        if (!acc[check.category]) acc[check.category] = [];
        acc[check.category].push(check);
        return acc;
      },
      {} as Record<string, typeof report.passed>,
    );
    ...
    Object.entries(grouped).forEach(([category, checks]) => {
      ...
      if (checks.length > 5) {
        console.log(
          `    ${dim(`... and ${checks.length - 5} more checks passed`)}`,
        );
      }
    });
  }
  ...
  if (report.summary.total === 0) {
    ...
    console.log(
      `  ${color("33", bold("Audit completed with " + report.summary.total + " issue(s)"))}`,
    );
  }
  console.log(bold("═".repeat(80)) + "\n");
}

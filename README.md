...
Create `.github/workflows/web-perf.yml`:
...
| -------------------- | ---------------------------------------- | -------- | --------------------- |
| `url`                | Target URL to audit                      | Yes      | -                     |
| `format`             | Output format: json, html, terminal, all | No       | `all`                 |
| `output-dir`         | Directory to save reports                | No       | `./web-perf-reports`  |
| `skip-audits`        | Comma-separated list of audits to skip   | No       | ``                    |
| `threshold-critical` | Max critical issues (0 = no limit)       | No       | `0`                   |
| `threshold-error`    | Max error issues (0 = no limit)          | No       | `0`                   |
| `threshold-warning`  | Max warning issues (0 = no limit)        | No       | `0`                   |
| `fail-on-threshold`  | Fail if thresholds exceeded              | No       | `true`                |
...
| Output           | Description                         |
| ---------------- | ----------------------------------- |
| `passed`         | Whether audit passed all thresholds |
...
| `info-count`     | Number of info issues               |
| `report-path`    | Path to generated report files      |

### Example Workflows

See [`.github/workflows/README.md`](.github/workflows/README.md) for complete examples:

- **Simple Audit:** Basic usage on push to main
- **PR Audit:** Comprehensive PR checks with comments
- **Scheduled Audit:** Daily/weekly monitoring
- **Deployment Audit:** Post-deployment verification
- **Custom Configuration:** All available options
